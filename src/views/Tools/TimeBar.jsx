...
class TimeBar extends React.Component {
  ...
  componentWillMount() {
    ...
    const monthFirstDate = new Date(
      `${curYear}-${intNum(curMonth)}-01T00:00:00`
    );
    const monthEndDate = new Date(
      `${isLastMonth ? curYear + 1 : curYear}-${intNum(
        (isLastMonth ? 0 : curMonth) + 1
      )}-01T00:00:00`
    );

    const fullYearDay = (endDate.getTime() - firstDate.getTime()) / fullDayLong;
    const yearDistDay = Math.ceil(
      (endDate.getTime() - curDate.getTime()) / fullDayLong
    );

    const fullMonthDay =
      (monthEndDate.getTime() - monthFirstDate.getTime()) / fullDayLong;
    const MonthDistDay = Math.ceil(
      (monthEndDate.getTime() - curDate.getTime()) / fullDayLong
    );

    const dayDistSecound =
      fullDayLong -
      (curDate.getTime() - new Date(curDate.setHours(0, 0, 0, 0)).getTime());
