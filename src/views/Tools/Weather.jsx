...
@observer class Weather extends React.Component {
  ...
  componentDidMount () {
    ...
    chart.line().position('week*temperature').color('type')
    chart.point().position('week*temperature').color('type').size(4).shape('circle').style({
      stroke: '#fff',
      lineWidth: 1
    })
                ...
                <tbody>
                  {
                    this.props.weatherDetails.map((item, i) => (
                      <tr key={i}>
                        <td>{item.date}</td>
                        <td>{item.wea} {item.air && `（空气污染：${item.air}）`}</td>
                        <td>{item.wind}</td>
                        <td>{item.sun}</td>
                      </tr>
                    ))
