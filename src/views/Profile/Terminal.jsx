...
class Terminal extends React.Component {
  ...
  keyupHandle(e) {
    ...
    if (e.key === "Enter") {
      if (this.step === 0) {
        ...
        const exactSize =
          isNum &&
          this.menuSelected * 1 <= hobbyData.list.length &&
          this.menuSelected * 1 > 0;
        if (exactSize) {
          this.step = 1;
  ...
  render() {
    ...
    const next = () => (
      <p key="continue">请点击窗口按Enter键继续...{cursor()}</p>
    );

    const inputNext = () => [
      <p className="gap" key="iptcontinue">
        请输入您选择的数字Enter键继续：
      </p>,
      <p key="iptNum">
        {inputNum()}
    ...
    const prev = () => (
      <p className="gap" key="back">
        请按Enter键返回到菜单...{cursor()}
      </p>
    );
      ...
      if (this.step === 0) {
        ...
        return [
          <p key="title" className="title">
            # {hobbyData.list[this.applySelected - 1]}
          </p>,
    ...
    return [
      <div className="profile-terminal" key="grid">
        <h1>
          ABOUT ME / <span>everything about me</span>
        </h1>
